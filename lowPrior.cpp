...
int main(int argc, char* argv[])
{
	...
	cl_command_queue_properties profilingQueueProperties[] = { CL_QUEUE_PROPERTIES, CL_QUEUE_PROFILING_ENABLE,
...
/// main body
	...
	const size_t maxSources = 57 ;//6*16;
        size_t sourcesCreated = 0;
	const cl_mem_flags memFlags = CL_MEM_READ_WRITE;

	std::cout << "\nLow Piority Queue: init buffs " << std::to_string(maxSources)<< " (" << std::to_string(buffSize/megaByte) << " MB)"  << std::endl;

#if 0
	const cl_mem source = clCreateBuffer(context, memFlags, buffSize, nullptr, &err);
	if (err != CL_SUCCESS) {
		std::cout << "failed to init queue. Error: " << std::to_string(err) << std::endl;
	}
#else
	...
	for(size_t i = 0; i < maxSources ; i++)
	{
        ...
        std::cout << "Create Time low:  " << std::to_string(microsecondTime) << std::endl ;
	...
	if(err == CL_SUCCESS)
	{
		...
		while( oLoop++ < maxLoops ) {
			...
			for(size_t iter = 0; iter < sourcesCreated;) {
				...
				std::cout << "low:  " << std::to_string(iter/3) << " time " << std::to_string(cmdQEnQTime) << std::endl ;
				clReleaseEvent(evt);
